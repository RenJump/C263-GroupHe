...
def feat_transform(data, info, label_encoder = None, encoders = None, cmax = None, cmin = None):
    ...
    #for idx, feature in enumerate(features):
        #print(f"Array at index {idx} has shape: {feature.shape}")
    features_expanded = [np.expand_dims(arr, axis=1) if arr.ndim == 1 else arr for arr in features]
    '''for idx, feature in enumerate(features_expanded):
        print(f"Array at index {idx} has shape: {feature.shape}")
        if idx == 0:
            print(feature)
        elif idx == 45:
    ...
    features = np.column_stack(features_expanded)
...
def _prepare_ml_problem(train, val, test, metadata, eval): 
    ...
    x_trains, y_trains = x_trains[x_trains.shape[0]//2:], y_trains[y_trains.shape[0]//2:]
